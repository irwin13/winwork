...
public class WebPage {
    ...
    public String readParameterSearchKeyword(HttpServletRequest request) {
        ...
        search = StringCommon.setSearchKeyword(search);
        return search;
    }
    ...
    private SortMethod setOrderMethod(String input) {
        if (Strings.isNullOrEmpty(input)) {
            return SortMethod.DESC;
        } else {
            if (input.equalsIgnoreCase(SortMethod.ASC.toString())) {
    ...
    public List<KeyValue> sortableField(Class<?> clazz) {
        ...
        for (Map.Entry<String, Class<?>> entry : sortableMap.entrySet()) {
            ...
            KeyValue keyValue = new KeyValue(entry.getKey(), StringCommon.insertStringInCamelCase(true, display, " "));
            result.add(keyValue);
        }
        return result;
    }
        ...
        if (model != null) {
            ...
            for (Field field : fieldArray) {
                ...
                for (int a = 0; a < annotationArray.length; a++) {
                    ...
                    if (annotation.annotationType().equals(ExcludeFormRead.class)) {
                        readField = false;
    ...
    public void setUserLogged(HttpServletRequest request, BaseEntity bean, boolean isOnlyUpdate) {

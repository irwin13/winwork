...
/**
 * @author irwin Timestamp : 31/01/13 15:18
 */
public class PojoUtil {
    ...
    /**
     * Populate an instance of an object from Map<String, String[]> using Apache BeanUtils.populate,
     * but we add custom DateConverter.
     * @param map
     * @param clazz
     * @param <T>
     * @return
     */
    public static <T> T populate(Map<String, String[]> map, Class<T> clazz) {
        T t = null;

        DateConverter dateConverter = new DateConverter();
        dateConverter.setPattern(WinWorkConstants.DATE_FORMAT_DEFAULT);
    ...
    public static SortParameter buildSortParameter(String columnName, String sortMethod) {
        ...
        if (!Strings.isNullOrEmpty(columnName) && !Strings.isNullOrEmpty(sortMethod)) {
            sortParameter = new SortParameter(columnName, SortMethod.valueOf(sortMethod));
        } else if (!Strings.isNullOrEmpty(columnName) && Strings.isNullOrEmpty(sortMethod)) {
            sortParameter = new SortParameter(columnName, SortMethod.ASC);
        }
        ...
        if (sortParameter != null) {
            if (!Strings.isNullOrEmpty(sortParameter.getColumnName())) {
                ...
                if (sortParameter.getSortMethod() != null) {
                    queryParam.append("&sortMethod=" + sortParameter.getSortMethod().name());
                } else {
                    queryParam.append("&sortMethod=" + SortMethod.ASC);
                }
            }
        }

...
/**
 * @author irwin Timestamp : 06/06/2014 13:12
 */
public class BatchInsertSteps {

    private static final Logger LOGGER = LoggerFactory.getLogger(BatchInsertSteps.class);
    private AppSettingDao dao = BatchTest.injector.getInstance(AppSettingDao.class);
    private PodamFactory podamFactory = new PodamFactoryImpl();

    private int loop;

    @Given("loop insert for $loop")
    public void given(int loop) {
        LOGGER.debug("loop = {}", loop);
        this.loop = loop;
    }

    @When("batch insert is executed with code $code")
